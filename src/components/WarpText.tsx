...
type ResolvedWarpTextProps = Required<Omit<WarpTextProps, 'style' | 'className'>> & {
  className: string
  style?: CSSProperties
}
...
const buildTextCanvas = ({
  ...
  lines.forEach((line, index) => {
    ...
    const x = props.textAlign === 'left' ? lineWidth / 2 : width / 2
    drawLine(ctx, line, x, startY + index * lineHeight, letterSpacing)
  })

  return canvas
}

const syncUniforms = (program: Program, props: ResolvedWarpTextProps) => {
...
export default function WarpText({
  ...
  const propsRef = useRef<ResolvedWarpTextProps>({
    text,
    color,
    fontSize,
    fontWeight,
  ...
  const contextRef = useRef<WarpContext | null>(null)
